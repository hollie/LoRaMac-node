 ...
 */
const double DaysInCentury = 36524.219;
...
void RtcClearStatus( void )
{
	RTC_ClearFlag(RTC_FLAG_ALRAF);			/* Clear RTC Alarm Flag */
	RTC_ITConfig( RTC_IT_ALRA, DISABLE );	/* Enable RTC Alarm A Interrupt */
	RTC_AlarmCmd( RTC_Alarm_A, DISABLE );	/* Enable the Alarm A */
}
...
void RtcStartWakeUpAlarm( uint32_t timeoutValue )
{
	...
	else if ( rtcAlarmDays > DaysInMonth[ RTC_DateStruct.RTC_Month - 1 ] )
	...
	RTC_ITConfig( RTC_IT_ALRA, ENABLE );	/* Enable RTC Alarm A Interrupt */
...
{   
	...
	{   
		...
		PWR_PVDCmd( DISABLE );			/* Disable the Power Voltage Detector */
		PWR_UltraLowPowerCmd( ENABLE ); /* Set MCU in ULP (Ultra Low Power) */
		PWR_FastWakeUpCmd( DISABLE );	/* Disable fast wakeUp */
